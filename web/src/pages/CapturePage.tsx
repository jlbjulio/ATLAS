  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      {!isInitialized && !initError && (
        <div
          className="flex items-center gap-3 rounded-lg border border-accent-200 bg-accent-50 px-4 py-3 text-sm text-accent-800"
          role="status"
        >
      ...
      <ol className="grid grid-cols-3 gap-2" aria-label="Flujo de captura">
        {["Observar", "Revisar extracción", "Confirmar"].map((step, index) => {
          ...
          return (
            <li
              key={step}
              className={`border-t-2 pt-2 text-xs font-medium ${active ? "border-primary-500 text-primary-700" : "border-surface-200 text-surface-400"}`}
            >
