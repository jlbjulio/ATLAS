...
export function Sidebar() {
  ...
  return (
    <aside className="hidden lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:w-64 lg:flex-col bg-white border-r border-surface-200">
      <div className="flex flex-col flex-1 overflow-y-auto">
        <div className="flex h-16 items-center border-b border-slate-800 bg-[#071a26] px-6">
          <Link
            to="/"
        ...
        >
          {navigation.map((item) => {
            ...
            return (
              ...
              >
                <item.icon
                  className={`w-5 h-5 ${isActive ? "text-primary-600" : "text-surface-400"}`}
                  aria-hidden="true"
                />
                {item.name}
                {item.badge && (
        ...
        <div className="border-t border-surface-200 p-4">
          <div className="rounded-lg border border-accent-200 bg-accent-50 p-3">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-accent-900">Operación local</p>
                <p className="mt-1 text-xs leading-5 text-accent-700">La evidencia permanece en el dispositivo.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-surface-200 p-4">
          ...
          <Link
            to="/help"
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-surface-600 hover:bg-surface-50 hover:text-surface-900 transition-colors mt-1"
          >
